...
export async function getAssignedCompaniesApi(ctx: any) {
  try {
    ...
    } = await ctx.clients.session.getSession(ctx.vtex.sessionToken, [
      'profile.email',
    ])
    ...
    const companies = await getAssignedCompanies(
      ctx,
      settings,
      profile?.email?.value
    )
    ...
    const schemaData = (
      await getAssignedCompaniesSchemaV2(ctx, settings)
    ...
    const schemaFromDataEntity = fields
      .filter((key: string) => key in properties)
      .reduce(
        (oldObject: any, key: string) => ({
          ...oldObject,
          [key]: properties[key],
        }),
        {}
