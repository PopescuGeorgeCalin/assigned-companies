...
export const getAssignedCompanies = async (
  ...
  const allFields = [...fields, ...urlFields.map((field: any) => field.name)]
  const {
    clients: { masterdata },
  } = ctx
...
export const formatRequestError = (error: any) => {
  if (error.response) {
     ...
     */
    return `Failed with status: ${
      error.response.status
    }; data: ${JSON.stringify(error.response.data)}; headers: ${JSON.stringify(
